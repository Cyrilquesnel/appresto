...
export function IngredientLinkModal({ platId, suggestions, onDone }: IngredientLinkModalProps) {
  ...
  const pending = suggestions.filter((s) => !dismissed.has(s.from_ingredient_id) && !confirmed.has(s.from_ingredient_id))
    ...
    return (
      <div className="fixed inset-0 bg-black/40 flex items-end justify-center z-50 p-4">
        <div className="bg-white rounded-2xl p-6 w-full max-w-md text-center space-y-4">
          ...
          <p className="text-sm text-gray-500">Les prix de la mercuriale seront utilisés pour calculer vos coûts.</p>
          <button
            onClick={onDone}
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-end justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-md space-y-4 p-6 max-h-[80vh] overflow-y-auto">
        <div>
          <h2 className="text-lg font-semibold text-gray-900">Lier les ingrédients à la mercuriale</h2>
          <p className="text-sm text-gray-500 mt-1">
            Ces ingrédients existent déjà dans votre mercuriale sous un nom différent. Confirmez le lien pour que les prix se mettent à jour automatiquement.
          </p>
        </div>

        <div className="space-y-3">
          {pending.map((s) => (
            <div key={s.from_ingredient_id} className="border border-gray-200 rounded-xl p-4 space-y-3">
              <div className="flex items-center gap-2 text-sm">
                <span className="font-medium text-gray-900 bg-gray-100 px-2 py-1 rounded">{s.from_nom}</span>
                <span className="text-gray-400">→</span>
                <span className="font-medium text-green-700 bg-green-50 px-2 py-1 rounded">{s.to_nom}</span>
                <span className="ml-auto text-xs text-gray-400">{Math.round(s.confidence * 100)}%</span>
              </div>
